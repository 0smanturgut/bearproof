...
// Daily twists: every build that has them contributes its own content.js, so the Worker names a day's twist
// with the exact code of the build that day is pinned to (worker/src/lib/twists.js).
const twistBuilds = [];
for (const b of builds) {
    const file = path.join(DIST, 'b', String(b.n), 'src/sim/content.js');
    if (!fs.existsSync(file)) continue;
    if (!fs.readFileSync(file, 'utf8').includes('export function dailyTwistForSeed')) continue;
    fs.copyFileSync(file, path.join(GEN, `content-b${b.n}.js`));
    twistBuilds.push(b.n);
}
fs.writeFileSync(
    path.join(GEN, 'twists.js'),
    '// generated by scripts/build.mjs\n' +
        twistBuilds.map((n) => `import * as b${n} from './content-b${n}.js';\n`).join('') +
        `export const TWIST_BUILDS = { ${twistBuilds.map((n) => `${n}: b${n}`).join(', ')} };\n`
);
